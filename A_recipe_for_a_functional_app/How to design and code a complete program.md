...
* データベース内の適切なユーザーレコードが新しい名前とメールアドレスで更新される
...
これらのエラーをどうやればエレガントな方法で処理できるのかを説明するというのが今回の1つのゴールです。

## 関数的に思考する ##

さて、ではユースケースの各ステップが把握できたわけですが、関数的なアプローチでどのように解決策を設計していけばよいのでしょうか？

兎にも角にも、オリジナルのダイアグラムと関数的な思考とのミスマッチを確認する必要があります。

ユースケースでは一般的なリクエスト/レスポンスモデルを考えています。
...
まず、関数型データとは前方のみにフローするということを認識する必要があります。
...
それがわかれば、今度はフロー全体を以下のように「ブラックボックス」関数へと変換します：
...
これで問題が解決できましたが、フロー中の処理毎にエラーを用意していたのでは脆弱な上に再利用性にも問題があります。
もっと改善できないものでしょうか？
...
実際、この型を利用するような便利な機能の揃ったすばらしいライブラリを紹介しますが、この型はありとあらゆるシナリオに対応できるものです。

ただしもう1つ考慮すべき点があります。
このままでは成功/失敗のステータスがわかるだけで、結果としては何のデータも得られません。
そこでもう少し手を加えて、成功または失敗時における実際のデータを持たせることができるようにしましょう。
単にジェネリック（あるいは型引数）を使用して成功型と失敗型を指定するだけです。
