...
これは``'a -> 'b``という関数引数と``Result<'a,'c>``の値を引数にとり、
``Result<'b,'c>``を返します。
...
* ただしモジュールの先頭や、関数内で演算子を定義する場合は例外とする。
  たとえば検証用のモジュールで``&&&``演算子を定義して、モジュール内でこの演算子を使うのは構わない。
