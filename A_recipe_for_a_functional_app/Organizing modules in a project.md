...
この記事ではそんなことになってしまわないように、コードを整理する簡単な方法を説明していきます。

<a name="note01">**</a>
``and``キーワードを使用することで相互参照を解決できる場合もありますが、推奨されません。
...
そして最も重要なこととして、ドメインレイヤーは永続化 (パーシステンス)レイヤーに依存しては **いけません** 。
 ["persistence agnostic (永続化の不可知論)"][link03] に従うべきです。
...
しかしこの記事はオブジェクト指向設計の機微をテーマにしているわけではないので、今のところはもっと単純なモデルを採用することにします。

## 型から挙動を分離する ##

**「10種のデータ構造を処理できる機能を10個用意するより、1種のデータ構造を処理できる機能を100個用意した方がよい」 アラン・パリス**

関数型デザイン(OOD: Objecct-oriented design)において、 **データと挙動を分離しておく** ことは非常に重要です。
...
F#プロジェクトの既知の制限として、フォルダ構造がサポートされておらず、そのために巨大なプロジェクトを整理できないのではないかということがよく話題になります。

純粋にオブジェクト指向デザインに従っているのであればこういった苦情が出るのももっともな話です。
しかしこれまでの説明からすれば、モジュールを直列的なリストにして依存性を正しく管理してやれば十分実用的であることがわかるはずです。
確かに理論上はファイルがバラバラになっていてもコンパイラが正しい順序を見つけ出すことが出来るはずですが、実際に正しい順序を見つけ出すのはそれほど簡単な話ではありません。

さらに重要なのは、 **人が** 正しい順序を見つけ出すことはさらに難しいわけで、必要以上に管理が大変になってしまうことでしょう。

巨大なプロジェクトの場合、現実的にはフォルダが無くても思うほどは問題になりません。
F#コンパイラ自身のように、この制限があるままでもきちんと統制されている巨大なF#プロジェクトも多々あります。
詳細については [cycles and modularity in the wild (現存プロジェクトの循環性とモジュール性)][link11] を参照してください。

### 型が相互依存している場合にはどうしたらいい？ ###

OOデザイン畑から移住してくると、以下のように相互依存している型が出てきてコンパイルできなくなることがあります：
